...
import {
  Pipeline, TriggerPipelineOnCallback, CallbackOnPipe, ConditionalNestedPipeline
} from '..'
...
it('conditional nested pipeline does not run in pipeline (boolean)', () => {
  ...
  mount(
    <Pipeline>
      ...
      <ConditionalNestedPipeline when='run_conditional' matches>
...
it('conditional nested pipeline runs in pipeline (string)', () => {
  ...
  mount(
    <Pipeline>
      <TriggerPipelineOnCallback callback={setTrigger} />
      <ConditionalNestedPipeline when='run_conditional' matches='true'>
        <CallbackOnPipe callback={() => Promise.resolve({ test: 'data' })} />
        <CallbackOnPipe callback={mockCallBack} />
      </ConditionalNestedPipeline>
    </Pipeline>
...
it('conditional nested pipeline runs in pipeline (number)', () => {
  ...
  mount(
    <Pipeline>
      <TriggerPipelineOnCallback callback={setTrigger} />
      <ConditionalNestedPipeline when='run_conditional' matches={123}>
        <CallbackOnPipe callback={() => Promise.resolve({ test: 'data' })} />
        <CallbackOnPipe callback={mockCallBack} />
      </ConditionalNestedPipeline>
    </Pipeline>
